...
def conduct_meta_mcs(par_file_skeleton, nb_iterations):
    ...
    par_file_skeleton : str
        The skeleton path to parameter files that should be used as 
        parameter files for the Monte Carlo Simulation.
    nb_iterations : int
    ...
    feather_files = [
        par_files[i].replace(
            'python/parameters/',
            output_dir + "/") for i in range(len(par_files))]
    feather_files = [
        feather_files[i].replace(
            '.json', '.feather') for i in range(len(par_files))]
    print(feather_files)
...
def aggregate_results(output_files, agg_filename):
    ...
    Takes the results as produced by the previous call of 
    `conduct_meta_mcs()`.
    Therefore, it should always be called from within this function or
    afterwards. It aggregates all the results for the feather files 
    passed as an input to this function.

    Parameters
    -----------
    output_files : list
        List with feather files containing the model output to be 
        aggregated.
    ...
    csv_files = [f.replace(".feather", ".csv") for f in feather_files]
    agg_filename_csv = agg_filename.replace(".feather", ".csv")

    results_csv = []
    for i in range(len(feather_files)):
        print(file_marker + str(i))
        y = pd.read_csv(csv_files[i])
        results_csv.append(y)

    overall_results_csv = pd.concat(
        results_csv, ignore_index=True, sort=False)
    opt_diff_results_csv = overall_results_csv
    opt_diff_results_csv.reset_index(inplace=True, drop=True)  

    opt_diff_results_csv.to_csv(agg_filename_csv)
